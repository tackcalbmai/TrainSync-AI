...
export default async function handler(req, res) {
  ...
  if (!requestId) return res.status(400).json({ error:"ADAPTATION_REQUEST_REQUIRED" });
  const request = await getRequest(token, user.id, requestId).catch(() => null);
  if (!request) return res.status(404).json({ error:"ADAPTATION_REQUEST_NOT_FOUND" });
  if (request.status !== "pending") return res.status(409).json({ error:"ADAPTATION_REQUEST_NOT_PENDING", status:request.status });
  if (request.request_type !== "load_options") return res.status(400).json({ error:"ADAPTATION_REQUEST_TYPE_UNSUPPORTED" });

  const loadsKg = cleanLoads(req.body?.loadsKg ?? req.body?.loads);
  if (!loadsKg.length) return res.status(400).json({ error:"LOAD_OPTIONS_REQUIRED", message:"Provide at least one available load in kilograms." });
  try {
    await saveLoadOptions(token, user.id, request.exercise_key, loadsKg);
    const result = await runProgramAdaptation({
      token,
      userId:user.id,
      workoutSessionId:request.source_workout_session_id,
      apply:req.body?.dryRun !== true,
      explicitAvailableLoadsByExercise:{ [request.exercise_key]:loadsKg },
    });
    return res.status(200).json({ saved:true, exerciseKey:request.exercise_key, loadsKg, adaptation:result });
  } catch (error) {
    return res.status(error.status || 500).json({ error:"ADAPTATION_INPUT_FAILED", message:error.message || "Could not apply adaptation input." });
  }
}
