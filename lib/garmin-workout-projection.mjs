...
import {
  GARMIN_OPEN_DURATION,
  assessGarminProjection,
  buildGarminOpenRangePreviewFromProjection,
} from "./garmin-target-policy.mjs";

export const GARMIN_WORKOUT_PROJECTION_VERSION = "fit-21.214.0+trainsync-2026-09-01.1";
...
export function garminFitProjectionReadiness(workout = {}) {
  ...
  if (!assessment.valid || !result.projection) {
    return {
      ready:false,
      publishReady:false,
      exactReady:false,
  ...
  return {
    ready:assessment.publishReady,
    publishReady:assessment.publishReady,
    exactReady:assessment.exactReady,
    rangePreviewAvailable:assessment.rangePreviewAvailable,
    deviceVerificationRequired:assessment.deviceVerificationRequired,
    reasonCode:assessment.reasonCode,
    targetPolicy:assessment.policy,
    targetPolicyVersion:assessment.policyVersion,
    candidatePolicy:assessment.candidatePolicy || null,
    rejectedAutomaticPolicies:assessment.rejectedAutomaticPolicies || [],
    ranges:assessment.ranges || [],
    fitProfileVersion:GARMIN_FIT_PROFILE_VERSION,
    exerciseMapVersion:GARMIN_EXERCISE_MAP_VERSION,
    projectionVersion:GARMIN_WORKOUT_PROJECTION_VERSION,
...
export function buildGarminOpenRangePreview(workout = {}) {
  return buildGarminOpenRangePreviewFromProjection(projectWorkoutToGarminFit(workout));
}
