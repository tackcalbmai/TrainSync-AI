...
function positiveInt(value) { if (value == null || value === "") return null; const n = Number(value); return Number.isFinite(n) && n > 0 ? Math.round(n) : null; }
function finiteRir(value) { if (value == null || value === "") return null; const n = Number(value); return Number.isFinite(n) && n >= 0 && n <= 6 ? n : null; }
...
export function targetForGarminSet(set, workout) {
  const empty = { metricType: set?.metricType === "duration_seconds" ? "duration_seconds" : "reps", targetReps: null, targetMinReps: null, targetMaxReps: null, targetDurationSeconds: null, targetMinDurationSeconds: null, targetMaxDurationSeconds: null, targetWeightKg: null, targetRir: null, prescribedSetCount:null, plannedExercise: null, plannedExerciseKey: null, similarity: 0 };
  ...
  const prescribedSetCount = Array.isArray(match.exercise?.sets) && match.exercise.sets.length ? match.exercise.sets.length : null;
  return { ...range, targetWeightKg: targetSet?.weightKg == null ? null : Number(targetSet.weightKg), targetRir: finiteRir(targetSet?.targetRir), prescribedSetCount, plannedExercise: match.exercise?.name || null, plannedExerciseKey: match.exercise?.exerciseKey || null, similarity: Math.round(match.score * 1000) / 1000 };
}
