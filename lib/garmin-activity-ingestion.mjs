...
export function targetForGarminSet(set, workout) {
  const empty = { metricType: set?.metricType === "duration_seconds" ? "duration_seconds" : "reps", targetReps: null, targetMinReps: null, targetMaxReps: null, targetDurationSeconds: null, targetMinDurationSeconds: null, targetMaxDurationSeconds: null, targetWeightKg: null, plannedExercise: null, similarity: 0 };
  ...
  return { ...range, targetWeightKg: targetSet?.weightKg == null ? null : Number(targetSet.weightKg), plannedExercise: match.exercise?.name || null, similarity: Math.round(match.score * 1000) / 1000 };
}
