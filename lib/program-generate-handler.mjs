...
async function trainingContext(token, userId) {
  try {
    const sq = new URLSearchParams({ select: "exercise_name,exercise_key,metric_type,reps,duration_seconds,weight_kg,rpe,target_reps,target_duration_seconds,target_weight_kg,is_warmup,completed_at", user_id: `eq.${userId}`, is_warmup: "eq.false", order: "completed_at.desc", limit: "300" });
...
function validateExerciseEquipment(draft, userProfile) {
  ...
  if (violations.length) {
    const detail = violations.slice(0, 4).map((item) => `${item.exercise}: ${item.missing.join(", ")}`).join("; ");
    throw Object.assign(new Error(`Program requires unavailable equipment (${detail}).`), { code: "PROGRAM_EQUIPMENT_MISMATCH", status: 422, violations });
  }
}
...
function normalizeDraft(draft, request, userProfile) {
  ...
  for (let week = 1; week <= durationWeeks; week += 1) {
    for (const template of templates) {
      sessions.push({
        weekIndex: week, dayIndex: template.dayIndex, slotIndex: 1, scheduledDate: weekDate(weekStart, week, template.dayIndex), title: template.title, status: "planned",
        payload: { title: template.title, focus: template.focus, estimatedDurationMinutes: template.estimatedDurationMinutes, exercises: template.exercises, progressionStrategy: draft.progressionStrategy, week },
        rationale: { source: "openai", evidenceVersion: EVIDENCE_VERSION, adaptive: true, note: week === 1 ? "Initial evidence-constrained microcycle." : "Scheduled from the stable microcycle; future prescriptions may adapt from completed performance." }
      });
    }
  }
  return {
    clientProgramId: programId, title: draft.title.trim(), summary: draft.summary.trim(), goal: request.goal, status: "draft", startDate: weekStart,
    durationWeeks, daysPerWeek: templates.length, defaultSessionMinutes: request.sessionMinutes || userProfile?.default_workout_minutes || 50,
    progressionStrategy: draft.progressionStrategy, priority: { text: request.priority || "", muscles: request.priorityMuscles || [] },
    settings: { availableDays: request.availableDays, timeEfficient: Boolean(request.timeEfficient), adaptive: true, preserveExerciseContinuity: true, trackBodyweightVariants: true, setMetrics: ["reps","duration_seconds"], equipmentContract: [...allowedEquipment(userProfile)], summary: draft.summary.trim() },
    evidenceVersion: EVIDENCE_VERSION, templates, sessions,
  };
}
...
async function generate(request, userProfile, history) {
  ...
  const instructions = [
    ...
    "For hypertrophy, use multiple sufficiently hard sets across practical rep ranges; most repetition-based working sets should normally stop around 1-3 RIR rather than requiring failure.",
    ...
    "Put priority exercises or muscles early in the session. Keep exercise selection stable and non-redundant.",
    ...
    "Every exercise must declare every required physical support/equipment item in requiredEquipment. Available equipment/support is authoritative. Never assume a bench, chair, table, low bar, rack, cable station, machine, or other support unless it appears in the allowed list. Floor and wall are always available.",
    "For repetition-based sets use setMetric=reps, non-null minReps/maxReps, and null minDurationSeconds/maxDurationSeconds. For timed isometrics use setMetric=duration_seconds, non-null minDurationSeconds/maxDurationSeconds, null minReps/maxReps, targetRir=null, and progressionMode=duration_progression or variant_progression. Never encode seconds as repetitions.",
    ...
    "Use progressionNote to state a concise measurable rule. For external load, usually add reps inside the range first and then the smallest practical load. For bodyweight, own the rep/time range before moving to a discrete harder named variation.",
    ...
    "If history is sparse, keep prescriptions conservative. Do not invent working weights. For a first exposure to an external-load movement, the athlete can calibrate the initial load from the prescribed rep range and effort; subsequent TrainSync sessions should then use recorded performance.",
    "Return exactly one session template for each requested training day, using those exact dayIndex values."
  ].join(" ");

  const response = await fetch(OPENAI_URL, { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, store: false, instructions, input: "Create the TrainSync multi-week program microcycle.", text: { format: programFormat } }), signal: AbortSignal.timeout(45000) });
  ...
  validateExerciseEquipment(draft, userProfile);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const validation = validateProgram({ title: program.title, goal: program.goal, durationWeeks: program.durationWeeks, daysPerWeek: program.daysPerWeek, defaultSessionMinutes: program.defaultSessionMinutes, priority: program.priority }, program.sessions);
    ...
    return res.status(200).json({ program, validation, ai: { mode: "openai", model: generated.model, responseId: generated.responseId, profileApplied: Boolean(userProfile), historyApplied: Boolean(history?.hasData) } });
  } catch (error) {
    return res.status(error.status === 429 ? 429 : error.status || 502).json({ error: error.code || "PROGRAM_GENERATION_FAILED", message: error.message || "Program generation failed.", ...(error.violations ? { violations: error.violations } : {}) });
  }
}
