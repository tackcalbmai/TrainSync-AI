...
const programFormat = {
  ...
  schema: {
    ...
    properties: {
      ...
      goal: { type: "string", enum: ["strength","hypertrophy","general_fitness","fat_loss","mixed"] },
        ...
        items: {
          ...
          properties: {
            ...
            exercises: {
              ...
              items: {
                type: "object", additionalProperties: false,
                required: ["name","exerciseKey","role","primaryMuscles","secondaryMuscles","notes","setCount","minReps","maxReps","targetRir","restSec","progressionNote","supersetGroup"],
                properties: {
...
function normalizeDraft(draft, request, userProfile) {
  ...
  const identity = JSON.stringify({ title: draft.title, weekStart, durationWeeks, templates });
...
function validateRequest(body, userProfile) {
  const goal = body.goal || userProfile?.goal || "general_fitness";
  ...
  const weekStart = /^\d{4}-\d{2}-\d{2}$/.test(body.weekStart || "") ? body.weekStart : isoDate(new Date());
  return {
    goal,
    durationWeeks,
    availableDays,
    ...
    priorityMuscles: (Array.isArray(body.priorityMuscles) ? body.priorityMuscles : []).filter((m) => MUSCLES.includes(m)),
...
async function generate(request, userProfile, history) {
  ...
  const instructions = [
    ...
    "For maximal strength, prioritize specific heavy compound work while retaining repetitions in reserve. For hypertrophy, use multiple sufficiently hard sets across practical rep ranges; most working sets should normally stop around 1-3 RIR rather than requiring failure.",
    ...
    "Every exercise must map to canonical primary and secondary muscles so TrainSync can deterministically audit weekly volume.",
    "Use progressionNote to state a concise rule such as: add reps inside the range first; add the smallest practical load only after all work sets reach the top of the range at target RIR.",
    request.timeEfficient ? "TIME-EFFICIENT MODE is enabled: use antagonist or non-competing supersets for suitable accessory/hypertrophy work. Do not superset the athlete's heavy priority strength lifts when doing so would compromise performance. Give paired exercises the same short supersetGroup label." : "Do not use supersets unless they clearly improve practicality; set supersetGroup to null otherwise.",
    ...
    request.priority ? `User priority: ${request.priority}.` : "No additional user priority was stated.",
    ...trainingContextInstructions(history),
    "If history is sparse, keep prescriptions conservative. Do not invent working weights; this schema intentionally prescribes rep ranges/RIR/rest rather than kilograms.",
