...
function first(value) {
  return asArray(value).find((item) => item != null);
}
...
function enumText(value) {
  ...
  if (Array.isArray(value)) return enumText(value[0]);
...
function activitySport(session, messages) {
  return enumText(session?.sport) || enumText(first(messages?.sportMesgs)?.sport) || null;
}

function activitySubSport(session, messages) {
  return enumText(session?.subSport) || enumText(first(messages?.sportMesgs)?.subSport) || null;
}

function strengthLike(sport, subSport) {
  const text = `${sport || ""} ${subSport || ""}`.toLowerCase();
  return text.includes("strength") || (text.includes("training") && text.includes("strength"));
}

function workoutStepMap(messages) {
  const map = new Map();
  for (const step of asArray(messages?.workoutStepMesgs)) {
    const index = numberOrNull(step?.messageIndex);
...
function setExerciseName(set, stepMap) {
  ...
  const stepName = humanize(step?.wktStepName || step?.notes);
  if (stepName) return stepName;

  const subtype = first(set?.categorySubtype);
  const subtypeName = humanize(subtype);
  if (subtypeName) return subtypeName;

  const category = first(set?.category);
...
function setTypeText(set) {
  return String(set?.setType ?? set?.type ?? "active").toLowerCase();
}

function isRestSet(set) {
  const type = setTypeText(set);
  return type === "rest" || type.includes("rest");
}

function isWarmupSet(set) {
  const type = setTypeText(set);
  return type.includes("warmup") || type.includes("warm_up");
}
...
function normalizeSetMessages(messages) {
  ...
  for (const raw of rawSets) {
    ...
    const reps = numberOrNull(raw.repetitions);
    ...
    if (groupingKey !== lastExerciseKey) {
      exerciseOrder += 1;
      setIndex = 1;
      lastExerciseKey = groupingKey;
    } else {
      setIndex += 1;
    }
    ...
    results.push({
      ...
      completedAt,
      garmin: {
        setType: enumText(raw.setType),
        category,
        categorySubtype: subtype,
        workoutStepIndex: stepIndex,
        durationSeconds: numberOrNull(raw.duration),
      },
    });
  }
...
export function decodeGarminFit(bytesInput) {
  ...
  if (decoded?.errors?.length) {
    const fatal = decoded.errors.find((error) => /fatal|crc|corrupt/i.test(String(error)));
    if (fatal) throw new Error(`FIT_DECODE_FAILED: ${fatal}`);
  }
  return decoded?.messages || {};
}

export function fitFileSha256(bytesInput) {
  const bytes = bytesInput instanceof Uint8Array ? bytesInput : Uint8Array.from(bytesInput || []);
...
export function normalizeGarminActivity(messages, { providerActivityId = null, fileHash = null } = {}) {
  ...
  const sport = activitySport(session, messages);
  const subSport = activitySubSport(session, messages);
  const startedAt = dateIso(session.startTime)
    || dateIso(sets[0]?.completedAt)
    || dateIso(activity.timestamp)
    || null;
  const completedAt = dateIso(session.timestamp)
    || dateIso(activity.timestamp)
    || dateIso(sets[sets.length - 1]?.completedAt)
    || startedAt;
  const durationSeconds = Math.max(0, Math.round(
    numberOrNull(session.totalTimerTime)
    ?? numberOrNull(session.totalElapsedTime)
    ?? 0
  ));
  ...
  return {
    ...
    summary: {
      ...
      totalVolumeKg: Math.round(sets.reduce((sum, set) => sum + ((set.weightKg || 0) * set.reps), 0) * 10) / 10,
      averageHeartRate: numberOrNull(session.avgHeartRate),
      maxHeartRate: numberOrNull(session.maxHeartRate),
      calories: numberOrNull(session.totalCalories),
    },
  };
}
