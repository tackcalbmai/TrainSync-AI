...
export function sessionVolumeKg(sets = []) {
  return round(sets.reduce((sum, set) => sum + setVolumeKg(set), 0), 1);
}

export function formatLoadKg(value) {
  const amount = Number(value);
  if (!Number.isFinite(amount)) return "—";
  return amount >= 1000 ? `${round(amount / 1000, 1)}t` : `${round(amount, 0)}kg`;
}
...
export function summarizeProgress(sessions = [], sets = [], options = {}) {
  ...
  const completedSets = sets
    .filter((set) => set?.completed_at && Number(set?.reps) > 0)
    .sort((a, b) => new Date(a.completed_at) - new Date(b.completed_at));

  const cutoff30 = shiftDays(now, -30);
  const cutoff56 = shiftDays(now, -56);
  const cutoff28 = shiftDays(now, -28);
  const cutoffPrev28 = shiftDays(now, -56);
  ...
  const current28Volume = completedSets
    .filter((set) => new Date(set.completed_at) >= cutoff28)
    .reduce((sum, set) => sum + setVolumeKg(set), 0);
  const previous28Volume = completedSets
    .filter((set) => {
      const date = new Date(set.completed_at);
      return date >= cutoffPrev28 && date < cutoff28;
    })
    .reduce((sum, set) => sum + setVolumeKg(set), 0);
  const trendPct = previous28Volume > 0
    ? round(((current28Volume - previous28Volume) / previous28Volume) * 100, 1)
    : null;
  ...
  for (const set of completedSets) {
    ...
    row.name = set.exercise_name || row.name;
    row.sets += 1;
    ...
    if (date >= cutoff56 && e1rm != null) {
      if (row.first56E1rmKg == null) row.first56E1rmKg = e1rm;
      row.best56E1rmKg = Math.max(row.best56E1rmKg || 0, e1rm);
    }

    const achievement = achievementMap.get(key);
    if (e1rm != null && e1rm > achievement.bestE1rm) {
      if (date >= cutoff30) achievement.lastPrAt = set.completed_at;
      achievement.bestE1rm = e1rm;
    }
    if (Number.isFinite(weight) && weight > achievement.bestWeight) {
      if (date >= cutoff30) achievement.lastWeightPrAt = set.completed_at;
      achievement.bestWeight = weight;
    }
  }
  ...
  const exercises = [...exerciseMap.values()]
    .map((row) => ({
      ...row,
      volumeKg: round(row.volumeKg, 1),
      change56Pct: row.first56E1rmKg && row.best56E1rmKg
        ? round(((row.best56E1rmKg - row.first56E1rmKg) / row.first56E1rmKg) * 100, 1)
        : null,
    }))
    .sort((a, b) => (b.bestE1rmKg || b.bestWeightKg || b.volumeKg) - (a.bestE1rmKg || a.bestWeightKg || a.volumeKg));

  const recentPrExercises = exercises.filter((row) => {
    const achievement = achievementMap.get(row.key);
    return achievement?.lastPrAt || achievement?.lastWeightPrAt;
  });
  ...
  return {
    ...
    volume30Kg: volume30,
    workSets30,
    duration30Seconds: duration30,
    trendPct,
    recentPrCount: recentPrExercises.length,
    exercises,
    weeklyVolume: weeklyVolumeBuckets(completedSets, now, 8),
    recentSessions: [...completed].sort((a, b) => new Date(b.completed_at) - new Date(a.completed_at)).slice(0, 8),
    hasData: completed.length > 0 || completedSets.length > 0,
    sets56Count: sets56.length,
  };
}
...
export function weeklyVolumeBuckets(sets = [], nowInput = new Date(), weeks = 8) {
  ...
  for (let index = weeks - 1; index >= 0; index -= 1) {
    const bucketEnd = shiftDays(end, -index * 7);
    const bucketStart = shiftDays(bucketEnd, -6);
    const volumeKg = sets.reduce((sum, set) => {
      const date = new Date(set.completed_at);
      return date >= startOfDay(bucketStart) && date <= endOfDay(bucketEnd) ? sum + setVolumeKg(set) : sum;
    }, 0);
    buckets.push({
      start: dateKey(bucketStart),
...
function shiftDays(dateInput, days) {
