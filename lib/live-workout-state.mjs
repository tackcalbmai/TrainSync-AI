export const LIVE_WORKOUT_STATE_VERSION = 2;
...
export function flattenWorkoutSets(workout) {
  ...
  exercises.forEach((exercise, exerciseIndex) => {
    ...
    sets.forEach((set, setIndex) => {
      ...
      const exerciseKey = String(exercise?.exerciseKey || "").trim() || null;
      const exerciseName = String(exercise?.name || `Exercise ${exerciseIndex + 1}`).trim();
      queue.push({
        ...
        exerciseKey,
        exerciseName,
        plannedExerciseKey:exerciseKey,
...
export function substitutePendingExercise(state, { exerciseOrder, replacement, assessment, approvalId = null, reason = "temporary_substitution" } = {}, now = new Date()) {
  const next = clone(state);
  const order = positiveInt(exerciseOrder);
  const replacementKey = String(replacement?.exerciseKey || replacement?.key || "").trim();
  const replacementName = String(replacement?.name || "").trim();
  if (!order || !replacementKey || !replacementName) throw new Error("LIVE_SUBSTITUTION_CONTEXT_REQUIRED");
  if (!assessment?.eligible || assessment?.candidateKey !== replacementKey || assessment?.loadTransferAllowed !== false) throw new Error("LIVE_SUBSTITUTION_NOT_APPROVED");
  const target = next.queue.filter((item) => item.exerciseOrder === order);
  if (!target.length) throw new Error("LIVE_SUBSTITUTION_EXERCISE_NOT_FOUND");
  const pending = target.filter((item) => item.status === "pending");
  if (!pending.length) throw new Error("LIVE_SUBSTITUTION_NO_PENDING_SETS");
  const plannedKey = target[0].plannedExerciseKey || target[0].exerciseKey;
  if (assessment.originalKey !== plannedKey && assessment.originalKey !== target[0].exerciseKey) throw new Error("LIVE_SUBSTITUTION_ORIGINAL_MISMATCH");
  const appliedAt = iso(now);
  const substitution = {
    approvalId:approvalId || null,
    exerciseOrder:order,
    plannedExerciseKey:plannedKey,
    plannedExerciseName:target[0].plannedExerciseName || target[0].exerciseName,
    replacementExerciseKey:replacementKey,
    replacementExerciseName:replacementName,
    reason:String(reason || "temporary_substitution"),
    score:Number(assessment.score || 0),
    tier:assessment.tier || null,
    policyVersion:assessment.policyVersion || null,
    catalogVersion:assessment.catalogVersion || null,
    warnings:arr(assessment.warnings),
    appliedAt,
  };
  for (const item of next.queue) {
    if (item.exerciseOrder !== order || item.status !== "pending") continue;
    item.exerciseKey = replacementKey;
    item.exerciseName = replacementName;
    item.group = arr(replacement.primaryMuscles).join(" + ") || item.group;
    item.targetWeightKg = null;
    item.substitution = clone(substitution);
  }
  next.substitutions = arr(next.substitutions).filter((item) => item.exerciseOrder !== order);
  next.substitutions.push(substitution);
  next.lastUpdatedAt = appliedAt;
  return next;
}
...
export function completedActualSets(state) {
  return (Array.isArray(state?.queue) ? state.queue : [])
    ...
    .map((item) => ({
      ...
      plannedExerciseKey:item.plannedExerciseKey || item.exerciseKey,
      plannedExerciseName:item.plannedExerciseName || item.exerciseName,
      substitution:item.substitution ? clone(item.substitution) : null,
      ...
      targetWeightKg:item.substitution ? null : item.targetWeightKg,
