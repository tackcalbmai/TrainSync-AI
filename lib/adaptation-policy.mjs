...
const PERFORMANCE_STATES = new Set(["insufficient_data", "high_effort_underperformance", "fatigue_signal", "underperformed", "on_target", "top_range_completed", "overperformed"]);

function normalizePerformanceState(state) {
  return state === "fatigue_signal" ? "high_effort_underperformance" : state;
}
...
function cleanPerformance(value) {
  ...
  return {
    state:normalizePerformanceState(value.state),
...
export function decideExerciseAdaptation({ progressionMode, recentPerformances = [], nextVariantKey = null } = {}) {
  ...
  const repeatedHighEffortUnderperformance = lastTwo.length >= 2 && lastTwo.every((x) => x.state === "high_effort_underperformance");
  const repeatedProblems = lastTwo.length >= 2 && lastTwo.every((x) => ["high_effort_underperformance", "underperformed"].includes(x.state));

  if (repeatedHighEffortUnderperformance) {
    return decision(
      "reduce_or_review",
      "REPEATED_HIGH_EFFORT_UNDERPERFORMANCE",
      "The prescribed target was missed at very high reported effort across repeated exposures. TrainSync treats this as an observed performance pattern, not a diagnosis of fatigue; a small reversible volume reduction or exercise-level review is justified.",
      ["reduceAfterRepeatedHighEffortUnderperformance"],
      lastTwo,
      { progressionMode:mode },
    );
  }
  if (repeatedProblems) {
    return decision("reduce_or_review", "REPEATED_UNDERPERFORMANCE", "Repeated underperformance is stronger evidence than one bad session; review load, volume, recovery and exercise fit before progressing.", ["reduceAfterRepeatedHighEffortUnderperformance"], lastTwo, { progressionMode:mode });
  }
  if (["high_effort_underperformance", "underperformed"].includes(latest.state)) {
