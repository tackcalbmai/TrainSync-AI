...
import { completedSetExerciseKey } from "./completed-set-identity.mjs";
import { sessionVolumeKg } from "./progress.mjs";
...
export async function consumeAuthRedirect(fragment = (typeof window !== "undefined" ? window.location.hash : "")) {
  ...
  clearAuthFragment();
  if (parsed.error) {
    const error = new Error(parsed.errorDescription);
  ...
  const provisional = {
    ...
    ...(parsed.expiresIn ? { expires_in:parsed.expiresIn, expires_at:Math.floor(Date.now() / 1000) + parsed.expiresIn } : {}),
  };
...
export async function requestPasswordReset(email, redirectTo) {
  ...
  const response = await fetch(`${SUPABASE_URL}/auth/v1/recover`, { method:"POST", headers:authHeaders(null), body:JSON.stringify(body) });
  return parseResponse(response);
}
export async function updatePassword(password) {
  const session = getSession();
  if (!session?.access_token) throw new Error("SIGN_IN_REQUIRED");
  const response = await fetch(`${SUPABASE_URL}/auth/v1/user`, { method:"PUT", headers:authHeaders(session), body:JSON.stringify({ password }) });
  const user = await parseResponse(response);
  if (user?.id) setSession({ ...session, user });
  return user;
}
...
export async function deleteCurrentUser() {
  const result = await authorizedFetch(`${SUPABASE_URL}/rest/v1/rpc/delete_current_user`, { method:"POST", body:"{}" });
  setSession(null);
  return result;
}

export async function saveWorkout(workout) {
  const session = getSession(), userId = session?.user?.id; if (!userId) throw new Error("SIGN_IN_REQUIRED");
  const row = { user_id: userId, client_workout_id: workout.id, revision: workout.revision || 1, title: workout.title, sport: workout.sport || "strength", scheduled_date: workout.scheduledDate, timezone: workout.timezone || "Europe/Riga", estimated_duration_minutes: workout.estimatedDurationMinutes, status: workout.status || "draft", payload: workout, updated_at: new Date().toISOString() };
  const data = await authorizedFetch(`${SUPABASE_URL}/rest/v1/workouts?on_conflict=user_id,client_workout_id,revision`, { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(row) });
...
export async function savePublication({ workoutDbId, workout, result }) {
  const session = getSession(), userId = session?.user?.id; if (!userId) throw new Error("SIGN_IN_REQUIRED");
  const row = { user_id: userId, workout_id: workoutDbId, provider: result.provider || "garmin", idempotency_key: result.idempotencyKey, provider_resource_id: result.providerResourceId, status: result.success ? "published" : "failed", error_code: result.code || null, response_metadata: result };
  const data = await authorizedFetch(`${SUPABASE_URL}/rest/v1/publication_attempts?on_conflict=user_id,idempotency_key`, { method: "POST", headers: { Prefer: "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify(row) });
  return Array.isArray(data) ? data[0] : data;
}
...
export async function saveProfile(profile) {
  const session = getSession(), userId = session?.user?.id; if (!userId) throw new Error("SIGN_IN_REQUIRED");
  const row = { user_id: userId, timezone: String(profile.timezone || "Europe/Riga").trim() || "Europe/Riga", units: profile.units === "imperial" ? "imperial" : "metric", goal: profile.goal || null, experience_level: profile.experience_level || null, default_workout_minutes: Math.max(15, Math.min(180, Number(profile.default_workout_minutes) || 50)), equipment: Array.isArray(profile.equipment) ? profile.equipment.filter(Boolean) : [], updated_at: new Date().toISOString() };
  const data = await authorizedFetch(`${SUPABASE_URL}/rest/v1/athlete_profiles?on_conflict=user_id`, { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(row) });
  return Array.isArray(data) ? data[0] : data;
}
...
function normalizeCompletedSet(set) {
  ...
  const reps = metricType === "reps" ? positiveInt(set?.reps) : null;
  const durationSeconds = metricType === "duration_seconds" ? positiveInt(set?.durationSeconds) : null;
  if (metricType === "reps" && reps == null) return null;
  if (metricType === "duration_seconds" && durationSeconds == null) return null;
  return {
    ...set, metricType, reps, durationSeconds,
    targetReps: positiveInt(set?.targetReps), targetMinReps: positiveInt(set?.targetMinReps ?? set?.minReps), targetMaxReps: positiveInt(set?.targetMaxReps ?? set?.maxReps),
    targetDurationSeconds: positiveInt(set?.targetDurationSeconds), targetMinDurationSeconds: positiveInt(set?.targetMinDurationSeconds ?? set?.minDurationSeconds), targetMaxDurationSeconds: positiveInt(set?.targetMaxDurationSeconds ?? set?.maxDurationSeconds),
    targetRir: boundedRir(set?.targetRir),
    weightKg: set?.weightKg === "" || set?.weightKg == null ? null : Math.max(0, Number(set.weightKg)), targetWeightKg: set?.targetWeightKg === "" || set?.targetWeightKg == null ? null : Math.max(0, Number(set.targetWeightKg)), rpe: set?.rpe === "" || set?.rpe == null ? null : Math.max(1, Math.min(10, Number(set.rpe))),
  };
}

export async function completeWorkoutSession({ workoutDbId = null, workout, sets, startedAt, completedAt, durationSeconds = 0, notes = "" }) {
  const session = getSession(), userId = session?.user?.id; if (!userId) throw new Error("SIGN_IN_REQUIRED"); if (!workout?.title) throw new Error("WORKOUT_REQUIRED");
  const completedSets = (Array.isArray(sets) ? sets : []).map(normalizeCompletedSet).filter(Boolean); if (!completedSets.length) throw new Error("LOG_AT_LEAST_ONE_SET");
  const finishedAt = completedAt || new Date().toISOString();
  const sessionRow = { user_id: userId, workout_id: workoutDbId || null, title: workout.title, started_at: startedAt || finishedAt, completed_at: finishedAt, duration_seconds: Math.max(0, Math.min(86400, Math.round(Number(durationSeconds) || 0))), status: "completed", notes: String(notes || "").trim() || null, total_sets: completedSets.length, total_volume_kg: sessionVolumeKg(completedSets), source: "trainsync", updated_at: new Date().toISOString() };
  const created = await authorizedFetch(`${SUPABASE_URL}/rest/v1/workout_sessions`, { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify(sessionRow) });
  const createdSession = Array.isArray(created) ? created[0] : created; if (!createdSession?.id) throw new Error("SESSION_CREATE_FAILED");
  const resultRows = completedSets.map((set) => ({
    user_id: userId, session_id: createdSession.id, exercise_name: String(set.exerciseName || "Exercise").trim(), exercise_key: completedSetExerciseKey(set), exercise_order: Math.max(1, Math.round(Number(set.exerciseOrder) || 1)), set_index: Math.max(1, Math.round(Number(set.setIndex) || 1)), metric_type: set.metricType,
    target_reps: set.metricType === "reps" ? set.targetReps : null, target_min_reps: set.metricType === "reps" ? set.targetMinReps : null, target_max_reps: set.metricType === "reps" ? set.targetMaxReps : null,
    target_duration_seconds: set.metricType === "duration_seconds" ? set.targetDurationSeconds : null, target_min_duration_seconds: set.metricType === "duration_seconds" ? set.targetMinDurationSeconds : null, target_max_duration_seconds: set.metricType === "duration_seconds" ? set.targetMaxDurationSeconds : null,
    target_weight_kg: set.targetWeightKg, target_rir: set.targetRir, reps: set.metricType === "reps" ? set.reps : null, duration_seconds: set.metricType === "duration_seconds" ? set.durationSeconds : null, weight_kg: set.weightKg, rpe: set.rpe, is_warmup: Boolean(set.isWarmup), completed_at: finishedAt,
  }));
  try { await authorizedFetch(`${SUPABASE_URL}/rest/v1/set_results`, { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify(resultRows) }); }
  catch (error) { try { await authorizedFetch(`${SUPABASE_URL}/rest/v1/workout_sessions?id=eq.${encodeURIComponent(createdSession.id)}`, { method: "DELETE" }); } catch {} throw error; }
  if (workoutDbId) { const completedWorkout = { ...workout, status: "completed" }; await updateWorkoutStatus(workoutDbId, "completed", completedWorkout); }
  return { session: createdSession, setResults: resultRows };
}

const SET_SELECT = "id,session_id,exercise_name,exercise_key,exercise_order,set_index,metric_type,target_reps,target_min_reps,target_max_reps,target_duration_seconds,target_min_duration_seconds,target_max_duration_seconds,target_weight_kg,target_rir,reps,duration_seconds,weight_kg,rpe,is_warmup,completed_at,created_at";
