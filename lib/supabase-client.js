...
function normalizeCompletedSet(set) {
  ...
  return {
    ...
    targetRir: boundedRir(set?.targetRir), rir: boundedRir(set?.rir),
...
export async function completeWorkoutSession({ workoutDbId = null, workout, sets, startedAt, completedAt, durationSeconds = 0, notes = "" }) {
  ...
  const resultRows = completedSets.map((set) => ({
    ...
    target_weight_kg: set.targetWeightKg, target_rir: set.targetRir, reps: set.metricType === "reps" ? set.reps : null, duration_seconds: set.metricType === "duration_seconds" ? set.durationSeconds : null, weight_kg: set.weightKg, rpe: set.rpe, rir: set.rir, is_warmup: Boolean(set.isWarmup), completed_at: finishedAt,
  }));
