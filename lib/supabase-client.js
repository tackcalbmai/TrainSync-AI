...
import { normalizeSessionMinutes, normalizeTimezone } from "./timezone.mjs";
...
export async function saveWorkout(workout) { const session=getSession(),userId=session?.user?.id; if(!userId) throw new Error("SIGN_IN_REQUIRED"); const row={user_id:userId,client_workout_id:workout.id,revision:workout.revision||1,title:workout.title,sport:workout.sport||"strength",scheduled_date:workout.scheduledDate,timezone:normalizeTimezone(workout.timezone,"UTC"),estimated_duration_minutes:workout.estimatedDurationMinutes,status:workout.status||"draft",payload:workout,updated_at:new Date().toISOString()}; const data=await authorizedFetch(`${SUPABASE_URL}/rest/v1/workouts?on_conflict=user_id,client_workout_id,revision`,{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify(row)}); return Array.isArray(data)?data[0]:data; }
...
export async function saveProfile(profile) { const session=getSession(),userId=session?.user?.id; if(!userId) throw new Error("SIGN_IN_REQUIRED"); const row={user_id:userId,timezone:normalizeTimezone(profile.timezone,"UTC"),units:profile.units==="imperial"?"imperial":"metric",goal:profile.goal||null,experience_level:profile.experience_level||null,default_workout_minutes:normalizeSessionMinutes(profile.default_workout_minutes),equipment:Array.isArray(profile.equipment)?profile.equipment.filter(Boolean):[],updated_at:new Date().toISOString()}; const data=await authorizedFetch(`${SUPABASE_URL}/rest/v1/athlete_profiles?on_conflict=user_id`,{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify(row)}); return Array.isArray(data)?data[0]:data; }
