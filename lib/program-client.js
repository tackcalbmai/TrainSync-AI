...
export async function listProgramAdjustments(programId, limit = 100) {
  const q = new URLSearchParams({
    select: "id,program_id,program_session_id,workout_session_id,adjustment_type,reason_code,reason_text,evidence_level,science_version,evidence_claim_ids,evidence_rule_keys,decision_confidence,metrics_snapshot,decision_source,before_state,after_state,created_at",
    program_id: `eq.${programId}`,
