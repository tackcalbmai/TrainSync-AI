...
export async function resolveAdaptationInput(requestId, loadsKg) {
  return apiRequest("/api/adapt-session", {
    method:"POST",
    body:JSON.stringify({ action:"resolve_input", requestId, loadsKg }),
  });
}

export async function acknowledgeAdaptationReview(requestId) {
  return apiRequest("/api/adapt-session", {
    method:"POST",
    body:JSON.stringify({ action:"acknowledge_review", requestId }),
  });
}
