...
export const SCIENTIFIC_AUDIT_VERSION = `${SCIENCE_VERSION}+adapt:${ADAPTATION_SCIENCE_VERSION}`;
...
function normalizeLegacyObservationLabels(value) {
  if (Array.isArray(value)) return value.map(normalizeLegacyObservationLabels);
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, normalizeLegacyObservationLabels(item)]));
  return value === "fatigue_signal" ? "high_effort_underperformance" : value;
}
...
export function buildProgramAdjustmentAudit({
  ...
  const normalizedMetrics = normalizeLegacyObservationLabels(object(metricsSnapshot));
  return {
    ...
    science_version: SCIENTIFIC_AUDIT_VERSION,
    ...
    metrics_snapshot: {
      ...normalizedMetrics,
      scienceVersions: {
        programming: SCIENCE_VERSION,
        adaptation: ADAPTATION_SCIENCE_VERSION,
      },
    },
...
export function validateProgramAdjustmentAudit(row = {}) {
  ...
  if (row.science_version !== SCIENTIFIC_AUDIT_VERSION) errors.push("science_version is not current");
  if (!SOURCES.has(row.decision_source)) errors.push("invalid decision_source");
  const versions = row?.metrics_snapshot?.scienceVersions;
  if (versions?.programming !== SCIENCE_VERSION) errors.push("programming science version missing or stale");
  if (versions?.adaptation !== ADAPTATION_SCIENCE_VERSION) errors.push("adaptation science version missing or stale");
  try {
