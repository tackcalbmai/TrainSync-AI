...
export function buildPostSessionAdaptationPlan({ completedProgramSession, completedWorkoutSession, setResults = [], futureProgramSessions = [], adjustmentHistory = [], explicitAvailableLoadsByExercise = {} } = {}) {
  ...
  for (const completedExercise of arr(completedProgramSession.payload?.exercises)) {
    ...
    const mutation = applyAdaptationDecision({ exercise:{ ...futureExercise, progressionMode }, decision, availableLoadsKg, lastObservedLoadKg:lastObservedLoad(history[0].rows) });
    if (!mutation.applied) {
