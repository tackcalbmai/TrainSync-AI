...
function latestTemporaryReduction(adjustmentHistory, exerciseKey) {
  const relevant = arr(adjustmentHistory)
    .filter((row) => row?.target_key === exerciseKey && ["REPEATED_FATIGUE_SIGNAL","RECOVERED_AFTER_VOLUME_REDUCTION"].includes(row?.reason_code))
    .sort((a,b) => isoTime(b.created_at) - isoTime(a.created_at));
...
function stableRecoverySignal(history) {
  ...
  return recentTwo.every((item) => {
    ...
    return ["on_target","top_range_completed","overperformed"].includes(p.state)
      && Number(p.confidence || 0) >= 0.6
      && !p.effortTooHigh;
  });
}
...
export function buildPostSessionAdaptationPlan({
  ...
  for (const completedExercise of arr(completedProgramSession.payload?.exercises)) {
    ...
    let decision = decideExerciseAdaptation({
      progressionMode,
