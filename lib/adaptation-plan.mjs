...
import { resolveRegisteredVariantTransition } from "./variant-progression.mjs";
...
const REDUCTION_REASON_CODES = new Set(["REPEATED_HIGH_EFFORT_UNDERPERFORMANCE", "REPEATED_FATIGUE_SIGNAL"]);
...
function workingSetCount(exercise) { return arr(exercise?.sets).filter((set) => !Boolean(set?.isWarmup ?? set?.is_warmup)).length; }
function normalizedObservationState(state) { return state === "fatigue_signal" ? "high_effort_underperformance" : state; }
...
function topRangeStatus(rows = []) {
  ...
  for (const row of rows) {
    const timed = String(row.metric_type || "reps") === "duration_seconds";
    const targetMax = timed ? finite(row.target_max_duration_seconds ?? row.target_duration_seconds) : finite(row.target_max_reps ?? row.target_reps);
...
export function classifyResultExposure(rows = []) {
  ...
  const prescribedSetCount = working.reduce((max, row) => Math.max(max, Math.round(finite(row.prescribed_set_count) || 0)), 0);
  ...
  result = { ...result, state:normalizedObservationState(result.state) };

  const top = topRangeStatus(working);
  const stableBase = !["high_effort_underperformance","underperformed","insufficient_data"].includes(result.state);
  if (stableBase && top.allTop && top.effortAboveTarget > 0) {
    result = { state:"on_target", confidence:0.72, effortObserved:true, effortMatchedTarget:false, effortTooHigh:true, effortComparedSets:top.effortComparable, effortToleranceRir:top.effortToleranceRir, reasons:["The top of the prescribed range was reached, but reported effort was materially higher than the prescribed RIR target; hold rather than progress."] };
  } else if (stableBase && top.allTop && top.allEffortMatched) {
    result = { state:"overperformed", confidence:0.8, effortObserved:true, effortMatchedTarget:true, effortTooHigh:false, effortComparedSets:top.effortComparable, effortToleranceRir:top.effortToleranceRir, reasons:["Every working set reached the top of its prescribed range while reported effort stayed within the prescribed RIR tolerance."] };
  } else if (stableBase && top.allTop && !top.highEffort) {
    result = top.exceeded > 0
      ? { state:"overperformed", confidence:0.76, effortObserved:top.effortObserved > 0, effortMatchedTarget:false, effortTooHigh:false, effortComparedSets:top.effortComparable, effortToleranceRir:top.effortToleranceRir, reasons:["Every working set reached the top of its prescribed range and at least one exceeded it without a recorded high-effort signal."] }
      : { state:"top_range_completed", confidence:0.66, effortObserved:top.effortObserved > 0, effortMatchedTarget:false, effortTooHigh:false, effortComparedSets:top.effortComparable, effortToleranceRir:top.effortToleranceRir, reasons:["Every working set reached the top of its prescribed range without complete effort confirmation."] };
  } else if (result.state === "overperformed" && !top.allTop) {
    result = { state:"on_target", confidence:0.7, effortObserved:top.effortObserved > 0, effortMatchedTarget:false, effortTooHigh:false, effortComparedSets:top.effortComparable, reasons:["Some work exceeded the range, but not every working set reached the top; keep the prescription stable."] };
  }
  ...
  if (prescribedSetCount > 0 && working.length < prescribedSetCount) {
    result = {
      state:"underperformed",
      confidence:Math.max(0.9, Number(result.confidence || 0)),
      effortObserved:top.effortObserved > 0,
      effortMatchedTarget:false,
      effortTooHigh:Boolean(result.effortTooHigh),
      effortComparedSets:top.effortComparable,
      prescribedSetCount,
      completedSetCount:working.length,
      reasons:[...(result.reasons || []), `Only ${working.length} of ${prescribedSetCount} prescribed working sets were recorded; incomplete work cannot be treated as a progression signal.`],
    };
  } else if (prescribedSetCount > 0) {
    result = { ...result, prescribedSetCount, completedSetCount:working.length };
  }
  return result;
}
...
function latestTemporaryReduction(adjustmentHistory, exerciseKey) {
  const relevant = arr(adjustmentHistory).filter((row) => row?.target_key === exerciseKey && (REDUCTION_REASON_CODES.has(row?.reason_code) || row?.reason_code === "RECOVERED_AFTER_VOLUME_REDUCTION")).sort((a,b) => isoTime(b.created_at) - isoTime(a.created_at));
  if (!relevant.length || !REDUCTION_REASON_CODES.has(relevant[0].reason_code)) return null;
  const reduction = relevant[0];
  const before = workingSetCount(reduction.before_state);
  const after = workingSetCount(reduction.after_state);
  return before > after ? { ...reduction, baselineWorkingSets:before, reducedWorkingSets:after } : null;
}

function stableRecoverySignal(history) {
  const recentTwo = arr(history).slice(0, 2);
  if (recentTwo.length < 2) return false;
  return recentTwo.every((item) => {
    const p = item?.performance || {};
    return ["on_target","top_range_completed","overperformed"].includes(p.state) && Number(p.confidence || 0) >= 0.6 && !p.effortTooHigh;
  });
}

function reviewMessage(item) {
  if (item?.mutation?.reasonCode === "MINIMUM_WORKING_SETS_REACHED") return "Repeated high-effort underperformance was observed, but TrainSync will not reduce this exercise below two working sets automatically. Review load selection, recovery, technique and exercise fit before making another change.";
...
function requirementsFromProposals(proposals = []) {
  ...
  for (const item of arr(proposals)) {
    ...
    if (INPUT_REQUIRED_MUTATION_REASONS.has(item?.mutation?.reasonCode)) {
      requirements.push({ type:"load_options", exerciseKey:item.exerciseKey, targetProgramSessionId:item.targetProgramSessionId, reasonCode:item.mutation.reasonCode, currentLoadKg:item.mutation.currentLoadKg ?? null, candidateLoadKg:item.mutation.candidateLoadKg ?? null, jumpRatio:item.mutation.jumpRatio ?? null, message:item.mutation.reasonCode === "LOAD_JUMP_TOO_LARGE_FOR_AUTO_APPLY" ? "The next known load jump is too large for automatic progression. Confirm a smaller available load or keep the current prescription." : "TrainSync needs a known next available load for this exercise before applying a load progression." });
      continue;
    }
    if (item?.decision?.action === "reduce_or_review") {
      requirements.push({ type:"review", exerciseKey:item.exerciseKey, targetProgramSessionId:item.targetProgramSessionId, reasonCode:item.mutation?.reasonCode || item.decision.reasonCode || "REVIEW_REQUIRED", decisionReasonCode:item.decision.reasonCode || null, decisionReasonText:item.decision.reasonText || null, beforeWorkingSets:item.mutation?.beforeWorkingSets ?? null, minimumWorkingSets:item.mutation?.minimumWorkingSets ?? null, message:reviewMessage(item) });
    }
  }
  return requirements;
}

export function buildPostSessionAdaptationPlan({ completedProgramSession, completedWorkoutSession, setResults = [], futureProgramSessions = [], adjustmentHistory = [], explicitAvailableLoadsByExercise = {}, allowedEquipment = [] } = {}) {
  ...
  for (const completedExercise of arr(completedProgramSession.payload?.exercises)) {
    ...
    const transitionResolution = progressionMode === "variant_progression" ? resolveRegisteredVariantTransition(exerciseKey, allowedEquipment) : null;
    const transition = transitionResolution?.resolved ? transitionResolution.nextExerciseKey : null;
    let decision = decideExerciseAdaptation({ progressionMode, recentPerformances:history.map((item) => item.performance), nextVariantKey:transition });
    if (progressionMode === "variant_progression" && decision.reasonCode === "VARIANT_LADDER_REQUIRED" && transitionResolution?.reasonCode === "VARIANT_EQUIPMENT_UNAVAILABLE") {
      decision = {
        ...decision,
        reasonCode:"VARIANT_EQUIPMENT_UNAVAILABLE",
        reasonText:"Progression is indicated, but the next registered exercise variant requires equipment that is not available in the athlete profile. Keep the current variant rather than inventing another movement.",
        requiredEquipment:transitionResolution.requiredEquipment || [],
      };
    }

    const activeReduction = latestTemporaryReduction(adjustmentHistory, exerciseKey);
    if (activeReduction && stableRecoverySignal(history) && workingSetCount(futureExercise) < activeReduction.baselineWorkingSets) {
      decision = {
        action:"restore_volume",
        reasonCode:"RECOVERED_AFTER_VOLUME_REDUCTION",
        reasonText:"Two consecutive stable exposures after a TrainSync performance-triggered volume reduction support restoring one working set toward the recorded pre-reduction baseline. The exact two-exposure threshold is a conservative product heuristic.",
        ruleKeys:["reduceAfterRepeatedHighEffortUnderperformance"],
        confidence:Math.min(...history.slice(0,2).map((item) => Number(item.performance?.confidence || 0.6))),
        progressionMode,
        targetWorkingSets:activeReduction.baselineWorkingSets,
        recoveryAdjustmentId:activeReduction.id,
      };
    }

    if (!["progress_load","progress_reps","progress_duration","progress_variant","reduce_or_review","restore_volume"].includes(decision.action)) continue;
    ...
    const mutation = applyAdaptationDecision({ exercise:{ ...futureExercise, progressionMode }, decision, availableLoadsKg, lastObservedLoadKg:lastObservedLoad(history[0].rows), allowedEquipment });
    if (!mutation.applied) {
    ...
    const metricsSnapshot = {
      ...
      recoveryAdjustmentId:decision.recoveryAdjustmentId || null,
      allowedEquipment:[...arr(allowedEquipment)],
      variantTransition:transitionResolution || null,
      recentPerformance:history.map((item) => ({
        sessionId:item.sessionId,
        completedAt:item.completedAt,
        state:normalizedObservationState(item.performance.state),
        ...
        effortTooHigh:Boolean(item.performance.effortTooHigh),
        effortComparedSets:Number(item.performance.effortComparedSets || 0),
        prescribedSetCount:Number(item.performance.prescribedSetCount || 0) || null,
        completedSetCount:Number(item.performance.completedSetCount || 0) || null,
    ...
    const audit = buildProgramAdjustmentAudit({ adjustmentType:decision.action, reasonCode:decision.reasonCode, reasonText:decision.reasonText, ruleKeys, beforeState:futureExercise, afterState:mutation.exercise, metricsSnapshot, decisionConfidence:decision.confidence, decisionSource:"deterministic" });
    proposals.push({ exerciseKey, applied:true, decision, mutation, targetProgramSessionId:nextSession.id, expectedRevision:Number(nextSession.revision || 1), newPayload, audit });
  }
