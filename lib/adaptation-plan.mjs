...
function latestTemporaryReduction(adjustmentHistory, exerciseKey) {
  const relevant = arr(adjustmentHistory).filter((row) => row?.target_key === exerciseKey && ["REPEATED_FATIGUE_SIGNAL","RECOVERED_AFTER_VOLUME_REDUCTION"].includes(row?.reason_code)).sort((a,b) => isoTime(b.created_at) - isoTime(a.created_at));
  if (!relevant.length || relevant[0].reason_code !== "REPEATED_FATIGUE_SIGNAL") return null;
  ...
  for (const completedExercise of arr(completedProgramSession.payload?.exercises)) {
    ...
    if (activeReduction && stableRecoverySignal(history) && workingSetCount(futureExercise) < activeReduction.baselineWorkingSets) {
      decision = {
        ...
        reasonText:"Two consecutive stable exposures after a TrainSync fatigue-driven volume reduction support restoring one working set toward the recorded pre-reduction baseline. The exact two-exposure threshold is a conservative product heuristic.",
