...
export default async function handler(req, res) {
  ...
  try {
    ...
    const status = error.status === 429 ? 429 : error.status || 502;
    return res.status(status).json({ error:error.code || "PROGRAM_GENERATION_FAILED", message:status === 429 ? "AI program generation is temporarily rate-limited. Try again later." : "AI program generation failed. Your saved program was not changed." });
  }
}
