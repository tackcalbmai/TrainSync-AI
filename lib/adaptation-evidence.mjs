...
export const ADAPTATION_SCIENCE_VERSION = "2026-08-23.4";
...
export const ADAPTATION_SOURCES = Object.freeze({
  ...
  varela_olalla_2025_fatigue: Object.freeze({
    ...
    limitations: "Primarily acute fatigue outcomes. It identifies volume, set duration and proximity to failure as fatigue determinants but does not establish a chronic individualized dose-reduction algorithm or prove that a specific performance miss was caused by fatigue.",
  }),
});
...
export const ADAPTATION_CLAIMS = Object.freeze({
  ...
  fixed_calendar_deload_is_not_established: Object.freeze({
    ...
    applicability: "Do not prescribe automatic calendar deloads as scientific necessity. Performance-triggered load/volume reductions remain a separate practical hypothesis requiring conservative use.",
  }),
  ...
  volume_is_a_plausible_fatigue_management_lever: Object.freeze({
    statement: "Resistance-training volume and proximity to failure materially influence acute fatigue, and autoregulating training volume is a defensible programming strategy. These findings do not establish that any particular high-effort performance miss was caused by fatigue, nor do they define one universal trigger or exact set reduction.",
    confidence: "moderate",
    sourceIds: ["hickmott_2022_autoreg_volume", "varela_olalla_2025_fatigue"],
    applicability: "Supports a small reversible volume adjustment after a repeated observed pattern of target misses at very high reported effort. TrainSync must describe the observation rather than diagnose its cause; the exact two-exposure trigger, one-set decrement, two-set floor and recovery threshold remain product heuristics.",
  }),
});
...
export const ADAPTATION_RULE_BINDINGS = Object.freeze({
  ...
  reduceAfterRepeatedHighEffortUnderperformance: Object.freeze({
    kind: "evidence_informed_heuristic",
    level: "heuristic",
    claimIds: ["volume_is_a_plausible_fatigue_management_lever"],
    note: "TrainSync may use a small reversible volume reduction after repeated target misses at very high reported effort. This is an observed performance pattern, not a diagnosis of fatigue. The exact rule of two consecutive exposures, one working set removed, a two-set floor and later one-set restoration remains a product heuristic.",
  }),
  reduceAfterRepeatedFatigue: Object.freeze({
    kind: "evidence_informed_heuristic",
    level: "heuristic",
    claimIds: ["volume_is_a_plausible_fatigue_management_lever"],
    note: "Legacy compatibility binding for audits created before the observed-state terminology migration. New decisions must use reduceAfterRepeatedHighEffortUnderperformance.",
  }),
