...
export const GARMIN_TARGET_POLICY_VERSION = "2026-08-23.1";
...
const OPEN_DURATION = Object.freeze({ id:5, name:"OPEN" });
...
export function assessGarminTargetPolicy(workout = {}) {
  const projected = projectWorkoutToGarminFit(workout);
  ...
  const rangeSteps = projected.projection.steps.filter(isRangeStep);
  const ranges = rangeSteps.map((step) => ({
  ...
  if (summary.rangeTargetSets > 0) {
    return {
      ...
      rejectedAutomaticPolicies:[
        { key:"use_upper_bound", reason:"Would turn a training target band into a mandatory maximum-rep instruction and can violate prescribed RIR." },
        { key:"use_lower_bound", reason:"Would discard valid progression room inside the prescribed target band." },
        { key:"use_midpoint", reason:"Has no established physiological or FIT-contract basis and still replaces a range with a false exact target." },
      ],
      candidatePolicy:{
...
export function buildGarminOpenRangePreview(workout = {}) {
  const assessment = assessGarminTargetPolicy(workout);
  const projected = projectWorkoutToGarminFit(workout);
  ...
  projection.steps = projection.steps.map((step) => {
    ...
    return {
      ...step,
      duration:{ type:{ ...OPEN_DURATION }, seconds:null, reps:null },
