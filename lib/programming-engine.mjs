...
export const RULES = Object.freeze({
  ...
  competingSuperset: { evidence: "moderate", source: "Superset systematic review/meta-analysis 2025" },
...
const MUSCLE_ALIASES = new Map([
  ...
  ["forearms", "forearms"], ["abs", "abs"], ["core", "core"],
...
export function estimateSessionMinutes(session) {
  ...
  for (const exercise of singles) {
    ...
    if (!count) continue;
    seconds += 75; // equipment/setup transition
  ...
  for (const groupExercises of paired.values()) {
    ...
    seconds += 60 * usable.length; // setup for paired stations
    ...
    for (let roundIndex = 0; roundIndex < rounds; roundIndex += 1) {
      ...
      for (const exercise of usable) {
        ...
        // Short movement transition inside the pair. Main prescribed recovery follows the round.
        seconds += 15;
  ...
  for (const [group, members] of paired.entries()) {
    ...
    for (let i = 0; i < members.length; i += 1) {
      for (let j = i + 1; j < members.length; j += 1) {
        const left = primaryMuscles(members[i]);
        const right = primaryMuscles(members[j]);
        const overlap = [...left].filter((muscle) => right.has(muscle));
        if (overlap.length) {
          pushFinding(warnings, "warning", "COMPETING_SUPERSET", `Superset ${group} directly targets ${overlap.join(", ")} in both exercises; this pairing may reduce repetitions or load compared with non-competing pairing.`, { group, muscles: overlap, exercises: [members[i].name, members[j].name] }, RULES.competingSuperset.evidence);
        }
      }
    }
  }
...
export function validateProgram(program, sessions = []) {
  ...
  for (const [week, weekSessions] of [...byWeek.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    weekSummaries.push({ week, sessionCount: weekSessions.length, dose });
  ...
  if (priorityMuscles.length && weekSummaries.length) {
    for (const summary of weekSummaries) {
      for (const muscle of priorityMuscles) {
        const value = summary.dose.muscles.fractional[muscle] || 0;
        if (value === 0) pushFinding(errors, "error", "PRIORITY_MUSCLE_MISSING", `${muscle} is marked as a priority but receives no work in week ${summary.week}.`, { week: summary.week, muscle });
      }
    }
  }
