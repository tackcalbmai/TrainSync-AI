...
// Hard structure, evidence-informed warnings and product heuristics stay separate.

export const EVIDENCE_VERSION = "2026-08-23";
...
const LEGACY_FATIGUE_MUSCLE_TAGS = Object.freeze({
  forearms: "grip",
  spinal_erectors: "spinal_bracing",
  abs: "core_bracing",
});
const LIMITING_FATIGUE_TAGS = new Set(["grip", "spinal_bracing", "core_bracing", "hinge"]);
...
function setMetric(set, exercise = null) {
  return String(set?.metricType ?? set?.metric_type ?? exercise?.setMetric ?? exercise?.set_metric ?? "reps").toLowerCase();
}
function setWorkSeconds(exercise, index) {
  const prescription = setPrescription(exercise, index);
  if (setMetric(prescription, exercise) === "duration_seconds") {
    return Math.max(5, num(prescription.maxDurationSeconds ?? prescription.targetDurationSeconds ?? prescription.durationSeconds, 30));
  }
...
function fatigueTags(exercise) {
  const tags = new Set(arr(exercise?.fatigueTags ?? exercise?.fatigue_tags).map((value) => String(value || "").trim().toLowerCase()).filter(Boolean));
  // Legacy drafts encoded stabilizer fatigue as secondary muscle metadata.
  for (const muscle of allMuscles(exercise)) {
    const tag = LEGACY_FATIGUE_MUSCLE_TAGS[muscle];
    if (tag) tags.add(tag);
  }
  if (movementOf(exercise) === "hinge") tags.add("hinge");
  return tags;
}
...
function validateSupersets(exercises, warnings) {
  ...
  for (const [group, members] of paired.entries()) {
    ...
    if (strengthMembers.length >= 2) pushFinding(warnings, "warning", "HEAVY_PRIORITY_SUPERSET", `Superset ${group} pairs multiple strength-focused exercises; time savings may compromise priority performance.`, { group, exercises: strengthMembers.map((item) => item.name) }, RULES.competingSuperset.evidence);
    ...
    for (let i = 0; i < members.length; i += 1) {
      for (let j = i + 1; j < members.length; j += 1) {
        ...
        const leftFatigue = fatigueTags(a), rightFatigue = fatigueTags(b);
        const sharedFatigue = [...leftFatigue].filter((tag) => rightFatigue.has(tag) && LIMITING_FATIGUE_TAGS.has(tag));
        const compoundOrStrength = [a, b].some((exercise) => roleOf(exercise).includes("strength") || roleOf(exercise).includes("compound"));
        if (!primaryOverlap.length && sharedFatigue.length && compoundOrStrength) {
          pushFinding(warnings, "warning", "SHARED_LIMITER_SUPERSET", `Superset ${group} shares limiting fatigue (${sharedFatigue.join(", ")}) with a strength/compound movement; the pair may be locally non-competing but still degrade performance.`, { group, fatigueTags: sharedFatigue, exercises: [a.name, b.name] }, RULES.sharedFatigueSuperset.evidence);
        }
...
export function validateProgramSession(session, options = {}) {
  ...
  for (const [muscle, sets] of Object.entries(dose.muscles.fractional)) {
    if (sets > RULES.highPerSessionMuscleSets.value) pushFinding(warnings, "warning", "HIGH_SESSION_MUSCLE_VOLUME", `${muscle} receives ${sets} fractional sets in one session; returns may diminish and quality may fall.`, { muscle, fractionalSets: sets }, RULES.highPerSessionMuscleSets.evidence);
  }
  if (targetMinutes) {
...
export function validateProgram(program, sessions = []) {
  ...
  for (const session of arr(sessions)) {
    ...
    errors.push(...result.errors.map((finding) => ({ ...finding, context: { ...finding.context, week, sessionId: session.id || null } })));
    warnings.push(...result.warnings.map((finding) => ({ ...finding, context: { ...finding.context, week, sessionId: session.id || null } })));
  }
  ...
  for (const [week, weekSessions] of [...byWeek.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    const dated = weekSessions
      .map((session) => ({ session, time: dateMs(session.scheduled_date ?? session.scheduledDate), dose: sessionDose(session) }))
...
function targetMetric(target) {
  return String(target?.metricType ?? target?.metric_type ?? (target?.durationSeconds != null || target?.targetDurationSeconds != null ? "duration_seconds" : "reps")).toLowerCase();
}
function actualMetric(actual) {
  return String(actual?.metricType ?? actual?.metric_type ?? (actual?.durationSeconds != null ? "duration_seconds" : "reps")).toLowerCase();
}
