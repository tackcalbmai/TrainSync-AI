...
export function estimateSessionMinutes(session) {
  ...
  let seconds = 5 * 60; // general transition + exercise-specific ramping allowance
  for (const exercise of exercises) {
    const count = setCount(exercise);
    if (!count) continue;
    seconds += 75; // equipment/setup/transition heuristic
    for (let i = 0; i < count; i += 1) {
      const prescription = setPrescription(exercise, i);
      ...
      seconds += Math.max(15, maxReps * repSeconds);
      if (i < count - 1) seconds += Math.max(0, num(prescription.restSec ?? exercise.restSec, 90));
    }
  }
...
export function validateProgram(program, sessions = []) {
  ...
  for (const [week, weekSessions] of [...byWeek.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    // Detect very close repeat exposure only when dates exist and the muscle receives a meaningful dose in both sessions.
...
export function classifyExercisePerformance({ targetSets = [], actualSets = [] } = {}) {
  ...
  let met = 0, missed = 0, highEffort = 0, over = 0;
