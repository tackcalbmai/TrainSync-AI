...
export function exerciseKey(value) {
  return String(value || "exercise").toLowerCase().normalize("NFKD")
    .replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "exercise";
}
...
export function fractionalMuscleVolume(exercises = []) {
  ...
  for (const exercise of arr(exercises)) {
    ...
    for (const muscle of primary) {
      ...
      fractional[muscle] = (fractional[muscle] || 0) + sets;
    }
  ...
  for (const table of [direct, indirect, fractional]) {
    for (const key of Object.keys(table)) table[key] = round(table[key], 1);
  }
  return { direct, indirect, fractional };
}

function setPrescription(exercise, index) {
  if (Array.isArray(exercise?.sets)) return exercise.sets[index] || exercise.sets[0] || {};
...
function setRestSeconds(exercise, index) {
  const prescription = setPrescription(exercise, index);
  return Math.max(0, num(prescription.restSec ?? exercise.restSec, 90));
}
...
export function estimateSessionMinutes(session) {
  ...
  let seconds = 5 * 60; // general warm-up / transitions heuristic
  ...
  for (const exercise of singles) {
    ...
    for (let i = 0; i < count; i += 1) {
      seconds += setWorkSeconds(exercise, i);
      if (i < count - 1) seconds += setRestSeconds(exercise, i);
    }
  }
  ...
  for (const groupExercises of paired.values()) {
    ...
    for (let roundIndex = 0; roundIndex < rounds; roundIndex += 1) {
      ...
      for (const exercise of usable) {
        ...
        seconds += setWorkSeconds(exercise, roundIndex);
        seconds += 15;
...
export function sessionDose(session) {
  ...
  return {
    ...
    muscles: fractionalMuscleVolume(exercises),
    estimatedMinutes: estimateSessionMinutes({ ...session, exercises }),
  };
}
...
export function weeklyDose(sessions = []) {
  ...
  for (const session of arr(sessions)) {
    const dose = sessionDose(session);
    totalSets += dose.totalSets;
...
function roleOf(exercise) {
  return String(exercise?.role || "hypertrophy").toLowerCase();
}
function movementOf(exercise) {
  return String(exercise?.movementPattern || exercise?.movement_pattern || "").toLowerCase();
}
function loadTypeOf(exercise) {
  return String(exercise?.loadType || exercise?.load_type || "").toLowerCase();
}
function progressionModeOf(exercise) {
  return String(exercise?.progressionMode || exercise?.progression_mode || "").toLowerCase();
}

function pushFinding(list, severity, code, message, context = {}, evidence = "heuristic") {
  list.push({ severity, code, message, evidence, context });
}

function primaryMuscles(exercise) {
  return new Set(arr(exercise?.primaryMuscles ?? exercise?.primary_muscles).map(normalizeMuscle).filter(Boolean));
}
function secondaryMuscles(exercise) {
  return new Set(arr(exercise?.secondaryMuscles ?? exercise?.secondary_muscles).map(normalizeMuscle).filter(Boolean));
}
function allMuscles(exercise) {
  return new Set([...primaryMuscles(exercise), ...secondaryMuscles(exercise)]);
}
...
function validateSupersets(exercises, warnings) {
  ...
  for (const [group, members] of paired.entries()) {
    if (members.length < 2) {
      pushFinding(warnings, "warning", "ORPHAN_SUPERSET", `Superset ${group} contains only one exercise, so it provides no time-efficiency benefit.`, { group }, "heuristic");
      continue;
    }
    if (members.length > 3) {
      pushFinding(warnings, "warning", "OVERLOADED_SUPERSET", `Superset ${group} contains ${members.length} exercises; long circuits may make loading and equipment flow impractical.`, { group, exercises: members.length }, "heuristic");
    }
    const strengthMembers = members.filter((exercise) => roleOf(exercise).includes("strength"));
    if (strengthMembers.length >= 2) {
      pushFinding(warnings, "warning", "HEAVY_PRIORITY_SUPERSET", `Superset ${group} pairs multiple strength-focused exercises; time savings may compromise priority performance.`, { group, exercises: strengthMembers.map((x) => x.name) }, RULES.competingSuperset.evidence);
    }
    for (let i = 0; i < members.length; i += 1) {
      for (let j = i + 1; j < members.length; j += 1) {
        const a = members[i], b = members[j];
        const leftPrimary = primaryMuscles(a);
        const rightPrimary = primaryMuscles(b);
        const primaryOverlap = [...leftPrimary].filter((muscle) => rightPrimary.has(muscle));
        if (primaryOverlap.length) {
          pushFinding(warnings, "warning", "COMPETING_SUPERSET", `Superset ${group} directly targets ${primaryOverlap.join(", ")} in both exercises; this pairing may reduce repetitions or load compared with non-competing pairing.`, { group, muscles: primaryOverlap, exercises: [a.name, b.name] }, RULES.competingSuperset.evidence);
        }
        ...
        if (!primaryOverlap.length && limitingShared.length && compoundOrStrength) {
          pushFinding(warnings, "warning", "SHARED_LIMITER_SUPERSET", `Superset ${group} shares a likely limiting stabilizer (${limitingShared.join(", ")}) with a strength/compound movement; the pair may be locally non-competing but still degrade performance.`, { group, muscles: limitingShared, exercises: [a.name, b.name] }, RULES.sharedFatigueSuperset.evidence);
        }

        const aMove = movementOf(a), bMove = movementOf(b);
        if (aMove === "hinge" && bMove === "hinge") {
          pushFinding(warnings, "warning", "HINGE_SUPERSET", `Superset ${group} combines two hinge-pattern exercises; trunk, grip and posterior-chain fatigue may make this pairing inefficient.`, { group, exercises: [a.name, b.name] }, RULES.sharedFatigueSuperset.evidence);
        }
      }
    }
  }
...
export function validateProgramSession(session, options = {}) {
  ...
  for (const exercise of exercises) {
    ...
    if (!movementOf(exercise) || !loadTypeOf(exercise) || !progressionModeOf(exercise)) {
      pushFinding(warnings, "warning", "TRACKABILITY_METADATA_MISSING", `${exercise?.name || "Exercise"} is missing movement/load/progression metadata, so automatic adaptation will be less reliable.`, { exerciseKey: key }, "heuristic");
    }

    if (roleOf(exercise) === "primary_strength") {
      const maxReps = maxPrescribedReps(exercise);
      if (maxReps != null && maxReps > RULES.primaryStrengthMaxReps.value) {
        pushFinding(warnings, "warning", "PRIMARY_STRENGTH_REP_RANGE_BROAD", `${exercise?.name} is labeled primary strength but reaches ${maxReps} reps. A harder load or named bodyweight variation would usually make strength-specific tracking cleaner.`, { exerciseKey: key, maxReps }, RULES.primaryStrengthMaxReps.evidence);
      }
    }

    if (["bodyweight","assisted_bodyweight"].includes(loadTypeOf(exercise)) && progressionModeOf(exercise) === "load_progression") {
      pushFinding(warnings, "warning", "BODYWEIGHT_PROGRESSION_MISMATCH", `${exercise?.name} is bodyweight-based but uses load_progression. Use a trackable variant or reps-first progression unless external loading is explicitly part of the exercise.`, { exerciseKey: key }, "heuristic");
    }
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      if (minReps == null || minReps < 1 || maxReps == null || maxReps < minReps) {
        pushFinding(errors, "error", "REP_RANGE_INVALID", `${exercise?.name || "Exercise"} set ${i + 1} has an invalid rep prescription.`, { exerciseKey: key, set: i + 1 });
      }
      ...
      if (rest != null && role.includes("strength") && rest < RULES.heavyCompoundMinRestSec.value) {
        pushFinding(warnings, "warning", "HEAVY_REST_SHORT", `${exercise?.name} has ${rest}s rest; heavy priority work usually benefits from more recovery.`, { exerciseKey: key, restSec: rest }, RULES.heavyCompoundMinRestSec.evidence);
      } else if (rest != null && role.includes("compound") && rest < RULES.hypertrophyCompoundMinRestSec.value) {
        pushFinding(warnings, "warning", "COMPOUND_REST_SHORT", `${exercise?.name} has ${rest}s rest; this may reduce repetitions/load across sets.`, { exerciseKey: key, restSec: rest }, RULES.hypertrophyCompoundMinRestSec.evidence);
      }
      if (rir === 0 && role.includes("strength") && count >= 3) {
        pushFinding(warnings, "warning", "REPEATED_COMPOUND_FAILURE", `${exercise?.name} prescribes repeated failure on heavy strength work, increasing fatigue without a clear strength-specific need.`, { exerciseKey: key }, RULES.highEffortCompoundRir.evidence);
      }
    }
  }
  ...
  for (const [muscle, sets] of Object.entries(dose.muscles.fractional)) {
    if (sets > RULES.highPerSessionMuscleSets.value) {
      pushFinding(warnings, "warning", "HIGH_SESSION_MUSCLE_VOLUME", `${muscle} receives ${sets} fractional sets in one session; returns may diminish and quality may fall.`, { muscle, fractionalSets: sets }, RULES.highPerSessionMuscleSets.evidence);
    }
  }

  if (targetMinutes) {
    const delta = Math.abs(dose.estimatedMinutes - targetMinutes) / targetMinutes;
    if (delta > RULES.durationToleranceRatio.value) {
      pushFinding(warnings, "warning", "SESSION_TIME_MISMATCH", `Estimated duration is ~${dose.estimatedMinutes} min versus a ${targetMinutes} min target.`, { estimatedMinutes: dose.estimatedMinutes, targetMinutes }, RULES.durationToleranceRatio.evidence);
    }
  }
...
function movementCoverage(sessions) {
  ...
  for (const session of arr(sessions)) {
    for (const exercise of arr(session?.exercises ?? session?.payload?.exercises)) {
      const movement = movementOf(exercise);
      if (movement) patterns.add(movement);
    }
  }
  return {
    patterns: [...patterns],
    upperPush: patterns.has("horizontal_push") || patterns.has("vertical_push"),
    upperPull: patterns.has("horizontal_pull") || patterns.has("vertical_pull"),
    kneeDominant: patterns.has("squat") || patterns.has("lunge"),
    hinge: patterns.has("hinge"),
  };
}
...
export function validateProgram(program, sessions = []) {
  ...
  for (const [week, weekSessions] of [...byWeek.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    const dose = weeklyDose(weekSessions);
    const coverage = movementCoverage(weekSessions);
    ...
    if (["mixed","general_fitness","fat_loss"].includes(goal)) {
      ...
      if (missing.length) {
        pushFinding(warnings, "warning", "MOVEMENT_COVERAGE_GAP", `Week ${week} is missing ${missing.join(", ")} exposure for a ${goal} program.`, { week, missing }, "heuristic");
      }
    }
    ...
    for (let i = 1; i < dated.length; i += 1) {
      ...
      for (const muscle of Object.keys(curr.dose.muscles.direct)) {
        if ((curr.dose.muscles.direct[muscle] || 0) >= 3 && (prev.dose.muscles.direct[muscle] || 0) >= 3) {
          pushFinding(warnings, "warning", "DENSE_REPEAT_EXPOSURE", `${muscle} receives substantial direct work in sessions less than ${RULES.samePriorityMuscleMinGapHours.value}h apart.`, { week, muscle, gapHours: round(gapHours, 1) }, RULES.samePriorityMuscleMinGapHours.evidence);
        }
      }
    }
  }
  ...
  if (priorityMuscles.length && weekSummaries.length) {
    for (const summary of weekSummaries) {
      for (const muscle of priorityMuscles) {
        const fractional = summary.dose.muscles.fractional[muscle] || 0;
        const direct = summary.dose.muscles.direct[muscle] || 0;
        if (fractional === 0) {
          pushFinding(errors, "error", "PRIORITY_MUSCLE_MISSING", `${muscle} is marked as a priority but receives no work in week ${summary.week}.`, { week: summary.week, muscle });
        } else if (fractional < RULES.minimumPriorityFractionalSets.value) {
          pushFinding(warnings, "warning", "PRIORITY_STIMULUS_LOW", `${muscle} is marked as a priority but receives only ${fractional} fractional sets in week ${summary.week}.`, { week: summary.week, muscle, fractionalSets: fractional }, RULES.minimumPriorityFractionalSets.evidence);
        }
        if (fractional > 0 && direct === 0) {
          pushFinding(warnings, "warning", "PRIORITY_ONLY_INDIRECT", `${muscle} is marked as a priority but receives only indirect work in week ${summary.week}.`, { week: summary.week, muscle, fractionalSets: fractional }, "heuristic");
        }
      }
    }
  }
...
export function classifyExercisePerformance({ targetSets = [], actualSets = [] } = {}) {
  const targets = arr(targetSets), actual = arr(actualSets).filter((s) => num(s?.reps, 0) > 0);
  ...
  for (let i = 0; i < actual.length; i += 1) {
    ...
    const min = num(target.minReps ?? target.targetReps, null);
    const max = num(target.maxReps ?? target.targetReps, min);
    const rpe = num(act.rpe, null);
    const rir = num(act.rir, rpe == null ? null : 10 - rpe);
  ...
  if (missed > 0) return { state: "underperformed", confidence: round(0.55 + missed / actual.length * 0.3), reasons: ["One or more rep targets were missed."] };
