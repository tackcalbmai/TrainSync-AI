import { EXERCISE_CATALOG_VERSION, canonicalizeExerciseSelection } from "./exercise-catalog.mjs";

const DEFAULT_TIMEZONE = "Europe/Riga";

const exercise = (exerciseKey, setCount, reps, restSec, notes = "") => {
  const canonical = canonicalizeExerciseSelection({ exerciseKey, notes });
  if (!canonical) throw new Error(`Unknown demo exercise key: ${exerciseKey}`);
  return {
    ...canonical,
...
const TEMPLATES = {
  upper: {
    title: "Upper Body A",
    exercises: [
      exercise("barbell_bench_press", 4, 8, 120, "Leave 1–2 reps in reserve."),
      exercise("machine_row", 4, 8, 105),
      exercise("barbell_overhead_press", 3, 8, 105),
      exercise("lat_pulldown", 3, 10, 90),
      exercise("dumbbell_bench_press", 3, 10, 90),
      exercise("cable_biceps_curl", 3, 12, 75),
    ],
  },
  push: {
    title: "Push Strength",
    exercises: [
      exercise("barbell_bench_press", 4, 6, 150),
      exercise("dumbbell_bench_press", 3, 8, 105),
      exercise("barbell_overhead_press", 3, 8, 105),
      exercise("cable_chest_press", 3, 12, 75),
      exercise("dumbbell_lateral_raise", 3, 15, 60),
      exercise("cable_triceps_pressdown", 3, 12, 75),
    ],
  },
  pull: {
    title: "Pull Strength",
    exercises: [
      exercise("barbell_bent_over_row", 4, 8, 120),
      exercise("lat_pulldown", 4, 8, 105),
      exercise("seated_cable_row", 3, 10, 90),
      exercise("cable_face_pull", 3, 15, 60),
      exercise("dumbbell_curl", 3, 10, 75),
      exercise("cable_biceps_curl", 2, 12, 75),
    ],
  },
  legs: {
    title: "Lower Body A",
    exercises: [
      exercise("barbell_back_squat", 4, 6, 150, "Controlled depth and bracing."),
      exercise("barbell_romanian_deadlift", 3, 8, 120),
      exercise("leg_press", 3, 10, 105),
      exercise("seated_leg_curl", 3, 12, 75),
      exercise("leg_extension", 3, 12, 75),
      exercise("machine_calf_raise", 4, 12, 60),
    ],
  },
  full: {
    title: "Full Body",
    exercises: [
      exercise("barbell_back_squat", 3, 6, 135),
      exercise("barbell_bench_press", 3, 8, 120),
      exercise("machine_row", 3, 8, 105),
      exercise("barbell_romanian_deadlift", 3, 8, 120),
      exercise("barbell_overhead_press", 3, 8, 90),
      exercise("lat_pulldown", 3, 10, 90),
    ],
  },
};
...
export function validateWorkout(workout) {
  ...
  for (const [exerciseIndex, item] of (workout.exercises || []).entries()) {
    ...
    const canonical = canonicalizeExerciseSelection({ exerciseKey:item.exerciseKey, name:item.name });
    if (workout.exerciseCatalogEnforced && (!item.exerciseKey || canonical?.exerciseKey !== item.exerciseKey)) {
      errors.push({ code: "CANONICAL_EXERCISE_REQUIRED", message: `${label} does not have a valid canonical TrainSync exercise key.` });
    } else if (!canonical) {
      warnings.push({ code: "CANONICAL_EXERCISE_PENDING", message: `${label}: canonical exercise identity is missing, so substitution and adaptation are limited.` });
    }
  }
