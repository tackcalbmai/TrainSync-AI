...
export async function runProgramAdaptation({ token, userId, workoutSessionId, apply = true, explicitAvailableLoadsByExercise = {} } = {}) {
  ...
  if (!workoutSession) return { status:"skipped", reasonCode:"WORKOUT_SESSION_NOT_FOUND", appliedBundles:[] };
  if (workoutSession.status !== "completed") return { status:"skipped", reasonCode:"WORKOUT_SESSION_NOT_COMPLETED", appliedBundles:[] };
  if (!workoutSession.program_session_id) return { status:"skipped", reasonCode:"PROGRAM_SESSION_LINK_NOT_CONFIRMED", appliedBundles:[] };

  const programSession = await getProgramSession(token, userId, workoutSession.program_session_id);
  if (!programSession) return { status:"skipped", reasonCode:"PROGRAM_SESSION_NOT_FOUND", appliedBundles:[] };
  ...
  if (!plan.valid) return { status:"skipped", reasonCode:plan.reasonCode, plan, appliedBundles:[] };
  const bundles = bundleAdaptationProposals(plan.proposals);
  if (!bundles.length) return { status:"no_change", reasonCode:"NO_CONFIRMED_ADAPTATION", plan, bundles:[], appliedBundles:[] };
  if (!apply) return { status:"preview", reasonCode:"ADAPTATION_PREVIEW", plan, bundles, appliedBundles:[] };
