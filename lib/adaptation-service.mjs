...
import { normalizeSetResultEffortRows } from "./adaptation-effort.mjs";
...
async function getAthleteProfile(token, userId) {
  ...
  return one(token, `athlete_profiles?${q}`);
}
async function getSetHistory(token, userId) {
  const q = new URLSearchParams({ select:"session_id,exercise_name,exercise_key,set_index,prescribed_set_count,target_reps,target_min_reps,target_max_reps,target_weight_kg,target_rir,reps,weight_kg,rpe,rir,is_warmup,completed_at,metric_type,target_duration_seconds,target_min_duration_seconds,target_max_duration_seconds,duration_seconds", user_id:`eq.${userId}`, is_warmup:"eq.false", order:"completed_at.desc", limit:"500" });
  return normalizeSetResultEffortRows(await rest(token, `set_results?${q}`));
}
...
export async function runProgramAdaptation({ token, userId, workoutSessionId, apply = true, explicitAvailableLoadsByExercise = {} } = {}) {
  ...
  const [setResults, futureProgramSessions, storedLoadOptions, adjustmentHistory, athleteProfile] = await Promise.all([
    ...
    getAthleteProfile(token, userId).catch(() => null),
  ]);
  const availableLoadsByExercise = mergeLoadOptions(storedLoadOptions, explicitAvailableLoadsByExercise);
  const allowedEquipment = Array.isArray(athleteProfile?.equipment) ? athleteProfile.equipment : [];
  const plan = buildPostSessionAdaptationPlan({
    completedProgramSession:programSession,
    completedWorkoutSession:workoutSession,
    setResults,
    futureProgramSessions,
    adjustmentHistory,
    explicitAvailableLoadsByExercise:availableLoadsByExercise,
    allowedEquipment,
  });
