...
export async function runProgramAdaptation({ token, userId, workoutSessionId, apply = true, explicitAvailableLoadsByExercise = {} } = {}) {
  ...
  const [setResults, futureProgramSessions, storedLoadOptions] = await Promise.all([
  ...
  const plan = buildPostSessionAdaptationPlan({ completedProgramSession:programSession, completedWorkoutSession:workoutSession, setResults, futureProgramSessions, explicitAvailableLoadsByExercise:availableLoadsByExercise });
  ...
  if (!bundles.length) {
    if (plan.requirements?.length) {
      ...
      return { status:"needs_input", reasonCode:"ADAPTATION_INPUT_REQUIRED", plan, bundles:[], appliedBundles:[], requirements:plan.requirements };
    }
