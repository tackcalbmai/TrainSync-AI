...
export const MUTATION_POLICY_VERSION = "2026-08-23.4";
...
export const MIN_WORKING_SETS_AFTER_FATIGUE_REDUCTION = 2;
...
function reduceWorkingSetForRepeatedFatigue(exercise) {
  const next = clone(exercise);
  ...
  if (beforeWorkingSets <= MIN_WORKING_SETS_AFTER_FATIGUE_REDUCTION) {
    return {
      applied:false,
      ...
      beforeWorkingSets,
      minimumWorkingSets:MIN_WORKING_SETS_AFTER_FATIGUE_REDUCTION,
    };
  }
  ...
  return {
    applied:true,
    exercise:next,
    reasonCode:"WORKING_SET_REMOVED_FOR_REPEATED_FATIGUE",
    ruleKeys:["reduceAfterRepeatedFatigue"],
    mutation:{
      type:"volume_sets",
      temporary:true,
...
function restoreWorkingSetAfterRecovery(exercise, targetWorkingSets) {
  ...
  if (!target || working.length >= target) return { applied:false, reasonCode:"VOLUME_ALREADY_AT_BASELINE", exercise:next, ruleKeys:["reduceAfterRepeatedFatigue"] };
  if (!working.length) return { applied:false, reasonCode:"NO_WORKING_SET_TEMPLATE", exercise:next, ruleKeys:["reduceAfterRepeatedFatigue"] };
...
export function applyAdaptationDecision({ exercise, decision, availableLoadsKg = [], lastObservedLoadKg = null } = {}) {
  ...
  if (action === "reduce_or_review") {
    if (decision?.reasonCode === "REPEATED_FATIGUE_SIGNAL") return reduceWorkingSetForRepeatedFatigue(exercise);
