alter table public.set_results
  add column if not exists rir numeric null;

alter table public.set_results
  drop constraint if exists set_results_rir_check;

alter table public.set_results
  add constraint set_results_rir_check
  check (rir is null or (rir >= 0 and rir <= 6));

comment on column public.set_results.rir is
  'User-reported actual repetitions in reserve. Never inferred from Garmin sensor data.';
...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements(p_actual_sets) x(value)
    group by x.value->>'exerciseOrder', x.value->>'setIndex'
    having count(*) > 1
  ) then raise exception 'DUPLICATE_ACTUAL_SET_POSITION'; end if;

  select ws.id into v_ws_id
  from public.workout_sessions ws
  where ws.user_id = v_user_id and ws.program_session_id = p_program_session_id and ws.status = 'completed'
  limit 1;
  if v_ws_id is not null then
    return query select v_ws_id, true;
    return;
  end if;
  ...
  where ps.id = p_program_session_id
    ...
    and ps.workout_id = p_workout_id
    and ps.status in ('planned','generated')
  for update of ps;
  ...
  insert into public.workout_sessions (
    user_id, workout_id, program_session_id, title, started_at, completed_at,
    duration_seconds, status, notes, total_sets, total_volume_kg, source, updated_at
  ) values (
    v_user_id, p_workout_id, p_program_session_id, v_ps.title,
    coalesce(p_started_at, v_completed), v_completed,
    greatest(0, least(86400, coalesce(p_duration_seconds,0))),
    'completed', nullif(trim(coalesce(p_notes,'')),''), v_total_sets, 0,
    'trainsync', now()
  ...
  loop
    ...
    insert into public.set_results (
      user_id, session_id, exercise_name, exercise_key, exercise_order, set_index, prescribed_set_count, metric_type,
      ...
      v_user_id, v_ws_id,
      coalesce(nullif(v_plan_exercise->>'name',''), 'Exercise'),
      nullif(v_plan_exercise->>'exerciseKey',''),
      v_exercise_order, v_set_index, nullif(v_prescribed_sets,0), v_metric,
  ...
  where id = v_ws_id and user_id = v_user_id;

  update public.workouts
  set status='completed', payload=jsonb_set(coalesce(payload,'{}'::jsonb), '{status}', '"completed"'::jsonb, true), updated_at=now()
  where id=p_workout_id and user_id=v_user_id;

  update public.program_sessions
  set status='completed', rationale=coalesce(rationale,'{}'::jsonb) || jsonb_build_object('completionSource','trainsync','completedAt',v_completed), updated_at=now()
  where id=p_program_session_id and user_id=v_user_id;
